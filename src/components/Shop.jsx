...
function Shop() {
    ...
    if (productLoading) {
        ...
        content = productData?.data?.map((item, index) => (
            <div data-aos="fade-up"data-aos-anchor-placement="top-bottom" data-aos-delay="500" key={index} className='mt-5 mx-auto relative'>
                <div className="imgContainer">
                    ...
                    <div style={{ width: "300px", margin: '0 auto' }}>
                        <ul className='iconul flex flex-col justify-center items-center gap-8 rounded'>
                            <li><i onClick={()=> addToWishlist(item)} className="text-[20px] text-white fa-regular fa-heart"></i></li>
                ...
                {item?.isOnMarketStatus === 'pre-order' && <div className="absolute cursor-pointer right-2 top-0 w-[70px] h-[70px] rounded-full bg-white flex flex-col justify-center items-center text-gray-500 font-bold ">
                    <p>Up</p>
                    <p>Coming</p>
                </div>}
            </div>
    ...
    if (categoryLoading) {
        ...
        categoryContent = categoryData?.data?.map((item, index) => (
            <div key={index}>
                <Radio className="text-gray-400 text-[18px] mt-2 font-bold" value={item?._id}>{item?.name}</Radio>
            </div>
        ));
    }
    ...
    if (brandLoading) {
        ...
        brandContent = brandData?.data?.map((item, index) => (
            <div key={index}>
                <Radio className="text-gray-400 text-[18px] font-bold mt-2" value={item?._id}>{item?.name}</Radio>
            </div>
        ));
    }
    ...
    return (
        <div className='w-full'>
            <div className="container text-center my-8 mx-auto" data-aos="fade-up"data-aos-anchor-placement="top-bottom" data-aos-delay="500">
